...
namespace CodeAnalysis
{
	...
	bool doDisplay(ASTNode* pNode)
	{
		...
		for (std::string type : toDisplay)
		{
			...
			if (pNode->type_ == type && pNode->name_ != "Global Namespace")
				return true;
		}
		return false;
	}
	...
	void TypeAnal::DFS(ASTNode* pNode)
	{
		...
		{
			if (pNode->name_ != "Global Namespace")
				if (stack.size() != 0)
		...
		if (doDisplay(pNode))
		{
			if (stack.size() != 0 && pNode->type_ != "namespace")
				namespace_name = stack.top();
			if(pNode->name_ != "main")
		...
		{   //search only for global functions stop DFS inside class struct or main function.
			//if (pNode->type_ == "class" || pNode->type_ == "struct" ||pNode->name_ == "main")
				if ( pNode->type_ == "struct" || pNode->name_ == "main")
				memberfunc = true;
			if (!memberfunc)
				DFS(pChild);
		}
