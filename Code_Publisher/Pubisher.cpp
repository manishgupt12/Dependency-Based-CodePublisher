...
/////////////////////////////////////////////////////////////////////
//Make index file
bool Publish::Indexfile(Keys files_, Key Htm_location)
{
	...
	body += "\n<div class = \"indent\">";
	body += "\n<h4> All files </h4>";
	for (size_t t = 0; t < files_.size(); t++)  //add dependencies to file
	{
		std::string file_name = filename.getName(files_[t]);
		body += "\n<br><a href = " + file_name + ".html>" + files_[t] + "</a>" + "\n";
	}
	body += "\n</div>";
	body += "\n<hr/>";
	indexstr = body + "\n" + indexstr + "\n" + "</body>";
	...
	header += "\n</head>";
	indexstr = header + indexstr;
	updatefile(final_path, indexstr);
	return true;
}
...
bool Publish::addbodytag(std::string & strtxt, Key fileSpec, Keys Dep)
{
	...
	for (size_t t = 0; t < Dep.size(); t++)  //add dependencies to file
	{
		...
		body += "\n<br><a href = " + file_name + ".html>" + Dep[t] + "</a>" +"\n";
	}
	body += "\n</div>";
	body += "\n<hr/>";
