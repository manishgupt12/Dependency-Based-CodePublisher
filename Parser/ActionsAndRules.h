...
//  Author:		   Manish Gupta, magupta@syr.edu				   //
...
/*
  ...
  ====================
  ver 3.3: 03 Mar 2017
  - Added the action and rules for typedef, enum and aliases
  ver 3.3 : 26 Feb 2017
...
{  
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override{
      ...
      std::string name = (*pTc)[nameIndex];
	  if (nameIndex > 0 && (*pTc)[nameIndex - 1] == "~") { // is function a destructor?
        --nameIndex;
        name = "~" + name;      }
	  top->type_ = "function";
      top->name_ = name;
      ...
      size_t operIndex = pTc->find("operator");// is function an operator?
      ...
      if (nameIndex > 1 && (*pTc)[nameIndex - 1] == "::") {   // is function a member of a class or struct?
        std::string className = (*pTc)[nameIndex - 2]; //----< start find class name >--------------------
  ...
  ///////////////////////////////////////////////////////////////
  // rule to detect aliases statements
  ...
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Test aliases definition: ", *pTc);
  ...
  public:
	  ...
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Handle alia definition: ", *pTc);
		  p_Repos->currentAccess() = Access::publ;
  ...
  public:
	  ...
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  ...
		  p_Repos->currentAccess() = Access::publ;
		  size_t index1 = pTc->find("typedef")+1;
		  size_t index2 = pTc->find(";");
		  std::string name;
		  for (size_t i = index1; i <= index2;i++ )
		  {
  ...
  public:
    bool doTest(const Scanner::ITokCollection* pTc) override
    {
      ...
      if (tc[tc.length() - 1] == ";" && tc.length() > 2) {
        ...
        if (parentType != "function")    { // can't be executable so must be declaration
          doActions(pTc);
          return IRule::Stop;
        }
      }
